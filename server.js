...
app.get("/Sign-up", function (req, resp) {
    dbCon.query("insert into users(txtemail,Password,txtsel,dos,status) values(?,?,?,current_date(),1)", [req.query.kuchEmail, req.query.kuchpass, req.query.kuchsel], function (err, resultTable) {
        if (err == null) {
...
app.get("/log-in",function(req,resp){
    dbCon.query("select * from users where txtemail=?" , [req.query.kuchEmail] , function(err , resJSON){
            if(err==null){
                if(resJSON.length==1 && resJSON[0].status==1 && resJSON[0].Password==req.query.kuchpass){
                        resp.send(resJSON[0].txtsel);       
                }
                else if(resJSON.length==1 && resJSON[0].status==1 &&  resJSON[0].Password!=req.query.kuchpass){
...
app.get("/send-mail",function(req,resp){
    ...
    dbCon.query("select Password from users where txtemail=?",[req.query.kuchEmail2],function(err,password){
        if(err==null){
            ...
            subject: 'Welcome to Joy Ride-Reset Your Password',
